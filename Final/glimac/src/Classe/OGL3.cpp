...
GLint Modele::m_mvloc,Modele::m_mvploc,Modele::m_normalloc,Modele::m_textureloc;
...
void Modele::setTextureloc(GLint val){
	m_textureloc=val;
}
void Modele::init_uniformloc(Program* program){
	...
	setTextureloc(glGetUniformLocation(program->getGLId(),"uTexture"));
}
...
void Modele::initialize(const std::unique_ptr<Image>& typedetexture){
	glGenBuffers(1,&vbo);
    ...
    glTexImage2D(GL_TEXTURE_2D,0,GL_RGBA,typedetexture->getWidth(),typedetexture->getHeight(),0,GL_RGBA,GL_FLOAT,typedetexture->getPixels());
